...
type FuelLogRepository interface {
	...
	DeleteByBikeID(bikeID uuid.UUID) error
...
// It finds the log that comes immediately before the current one based on date and odometer
func (r *fuelLogRepository) FindPreviousLog(bikeID uuid.UUID, currentDate string, currentOdometer int) (*models.FuelLog, error) {
	var fuelLog models.FuelLog

	// Find the most recent log before this one
	// Either: date is earlier, OR same date but lower odometer reading
	err := r.db.Where("bike_id = ?", bikeID).
		Where("date::date < ?::date OR (date::date = ?::date AND odometer_reading < ?)", currentDate, currentDate, currentOdometer).
		Order("date DESC, odometer_reading DESC").
		Limit(1).
		First(&fuelLog).Error
...
// It finds the log that comes immediately after the current one based on date and odometer
func (r *fuelLogRepository) FindNextLog(bikeID uuid.UUID, currentDate string, currentOdometer int) (*models.FuelLog, error) {
	var fuelLog models.FuelLog

	// Find the next log after this one
	// Either: date is later, OR same date but higher odometer reading
	err := r.db.Where("bike_id = ?", bikeID).
		Where("date::date > ?::date OR (date::date = ?::date AND odometer_reading > ?)", currentDate, currentDate, currentOdometer).
...
// Update updates a fuel log (including pointer fields like mileage and distance_covered)
func (r *fuelLogRepository) Update(fuelLog *models.FuelLog) error {
	// Use Select("*").Updates() to ensure pointer fields are updated properly
	// Save() doesn't always update nil → value transitions for pointer fields
	return r.db.Model(fuelLog).Select("*").Updates(fuelLog).Error
}
...
// DeleteByBikeID soft deletes all fuel logs for a bike
func (r *fuelLogRepository) DeleteByBikeID(bikeID uuid.UUID) error {
	return r.db.Where("bike_id = ?", bikeID).Delete(&models.FuelLog{}).Error
}
